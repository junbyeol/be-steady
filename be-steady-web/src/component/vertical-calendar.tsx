...
export const VerticalCalendar: React.FC = () => {
  ...
  const showGoToToday = useMemo(() => {
    const monthDiff = (today.getFullYear() - viewDate.getFullYear()) * 12 + (today.getMonth() - viewDate.getMonth());
            ...
            {calendarData.map((item, i) => (
              <div
                ...
                className={`aspect-square flex flex-col text-sm rounded-lg transition-colors border-2 ${
                  ...
                  item.isCurrentMonth && item.day === today.getDate() && isCurrentMonth 
              ...
              >
                ...
                {/* {item.isCurrentMonth && <p className="text-xs text-blue-500 self-center">운동</p>} */}
              </div>
            ))}
          </div>
        </div>
